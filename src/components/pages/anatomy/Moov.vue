...
<template>
  <div class="d-flex align-center flex-column">
    ...
    <div class="text-subtitle-2 mt-2">СПИСОК ДВИЖЕНИЙ</div>
        </v-col>
      </v-row>
    ...
    <div v-for="item in items" :key="item">
      <v-card
        width="400"
        title=""
        :subtitle="item.MOOV_NOTES"
        text="подробнее"
        variant="outlined"
        class="mt-2"
        link
        @click="details(item.MOOV_ID)"
      ></v-card>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      search_string: "",
      items: null,
      url: 'api/v1/moov/search',
      details_path_name: 'moov-detail',
    };
  },
  computed: {},
  methods: {
