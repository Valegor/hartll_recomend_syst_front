...
<template>
  <div class="d-flex align-center flex-column">
    ...
    <div class="text-subtitle-2 mt-2">СПИСОК ОРГАНОВ</div>
        </v-col>
      </v-row>
    ...
    <div v-for="item in items" :key="item">
      <v-card
        width="400"
        title=""
        :subtitle="item.ORGAN_NAME"
        text="подробнее"
        variant="outlined"
        class="mt-2"
        link
        @click="details(item.ORGAN_ID)"
      ></v-card>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      search_string: "",
      items: null,
      url: 'api/v1/organ/search',
      details_path_name: 'organ-detail',
    };
  },
  computed: {},
  methods: {
