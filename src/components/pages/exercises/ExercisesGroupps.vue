...
<template>
  <div class="d-flex align-center flex-column">
    ...
    <div class="text-subtitle-2 mt-2">СПИСОК ГРУПП УПРАЖНЕНИЙ</div>
        </v-col>
      </v-row>
    ...
    <div v-for="item in items" :key="item">
      <v-card
        width="400"
        title=""
        :subtitle="item.EXGROUPP_NAME"
        text="подробнее"
        variant="outlined"
        class="mt-2"
        link
        @click="details(item.EXGROUPP_ID)"
      ></v-card>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      search_string: "",
      items: null,
      url: 'api/v1/exgroupp/search',
      details_path_name: 'exercise-groupp-detail',
    };
  },
  computed: {},
  methods: {
