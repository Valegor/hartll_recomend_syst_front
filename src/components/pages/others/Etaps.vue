  ...
  <div class="d-flex align-center flex-column">
    <v-row justify="center">
      <v-col cols="12" sm="12" md="12">
        <div class="text-subtitle-2 mt-2">СПИСОК ЭТАПОВ ТРЕНИРОВОЧНОГО ЦИКЛА</div>
      </v-col>
    </v-row>
    ...
    <div v-for="item in items" :key="item">
      <v-card width="400" title="" :subtitle="item.caption" text="подробнее" variant="outlined" class="mt-2" link
        @click="details(item.id)"></v-card>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      search_string: "",
      items: null,
      url: 'api/v1/etap/search',
      details_path_name: 'etap-detail',
    };
  },
  computed: {},
  methods: {
